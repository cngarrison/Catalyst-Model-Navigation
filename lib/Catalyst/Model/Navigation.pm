...
#  	$c->log->debug( "Value of am_items is: " . p(@am_items) ) if $c->debug;
...
#  	$c->log->debug( "Value of menu_parents is: " . p($menu_parents) ) if $c->debug;
	...
	for ( my $i = 0; $i <= $#$menu_parents; $i++ ) {
		...
		my $mp_ak = sprintf( '%s!%s', $parent_mp, $action_key );
		my $action_url_args = $action_attrs->{MenuArgs} || [];
		my $url;
		my $url_cb;
		if (scalar @$action_url_args >= 1) {
			$url_cb = $self->_build_url_coderef($action, @$action_url_args),
		} else {
		...
		my $item = {
			...
			description => $c_nav_config->{$mp_ak}->{description}
			  || $ctr_nav_config->{$mp_ak}->{description}
			  || $action_attrs->{MenuDescription}->[$i]
			  || $last_item->{description}
			  || '',
			required_roles => $c_nav_config->{$mp_ak}->{required_roles}
			  || $ctr_nav_config->{$mp_ak}->{required_roles}
			  || $action_attrs->{MenuRoles}->[$i]
...
sub _build_url_coderef {
	...
	my $sub = sub {
		...
		$ctx->uri_for_action( $action, [@action_args] );
	};
	return $sub;
